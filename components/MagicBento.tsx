...
const ParticleCard: React.FC<any> = ({ children, className = '', disableAnimations = false, style, particleCount = DEFAULT_PARTICLE_COUNT, glowColor = DEFAULT_GLOW_COLOR, enableTilt = true, clickEffect = false, enableMagnetism = false }) => {
  ...
  return (
    <div ref={cardRef as any} className={`${className} particle-container`} style={{ ...(style || {}), position: 'relative', overflow: 'hidden' }}>
      {children}
    </div>
  );
};
...
const MagicBento: React.FC<any> = ({ textAutoHide = true, enableStars = true, enableSpotlight = true, enableBorderGlow = true, disableAnimations = false, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, particleCount = DEFAULT_PARTICLE_COUNT, enableTilt = false, glowColor = DEFAULT_GLOW_COLOR, clickEffect = true, enableMagnetism = true }) => {
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          ...
          const cardProps: any = { className: baseClassName, style: { backgroundColor: card.color, ['--glow-color' as any]: glowColor } };
          ...
          return (
            <div key={index} {...cardProps} ref={el => { if (!el) return; /* attach interactions if needed */ }}>
              <div className="magic-bento-card__header"><div className="magic-bento-card__label">{card.label}</div></div>
              <div className="magic-bento-card__content"><h2 className="magic-bento-card__title">{card.title}</h2><p className="magic-bento-card__description">{card.description}</p></div>
            </div>
          );
